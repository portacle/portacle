...
function ensure-git-platform() {
    ...
    case "$PLATFORM" in
        win) ensure-installed "$SHARED_BIN_DIR/" $(win-exes-for-package openssh)
             ...
             ;;
        lin) ensure-installed "$SHARED_LIB_DIR/" "/usr/lib/libcurl.so" /lib/libnss_*.so.2 /lib64/libnss_*.so.2 /lib/x86_64-linux-gnu/libnss_*.so.2
             ensure-dependencies "/usr/lib/libcurl.so" /lib/libnss_*.so /lib64/libnss_*.so /lib/x86_64-linux-gnu/libnss_*.so
