...
function certify() {
    ...
    codesign -s "$cert" \
             ...
             "${exes[@]}" "${libs[@]}" "$INSTALL_DIR/Portacle.app" "$INSTALL_DIR/mac/sbcl/lib/sbcl/sbcl.core"
}
    ...
    case "$PLATFORM" in
        mac)
            if security find-identity -p codesigning -v | grep "$CERT_CN"; then
                ...
                if ! security find-identity -p codesigning -v | grep "portacle"; then
